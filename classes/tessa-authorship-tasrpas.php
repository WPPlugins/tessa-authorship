...
class tasrpas_worker {
	...
	{
		global $tessa_authorship, $tasrpas_options, $wpdb;
		...
		if ( class_exists('SitePress') && defined('ICL_LANGUAGE_CODE') ) :
			...
			$query_str = "
				SELECT $wpdb->posts.ID, $wpdb->posts.post_title, $wpdb->posts.post_status, $wpdb->posts.post_date, $wpdb->posts.post_type, {$db_icltable}.language_code
				FROM $wpdb->posts, $db_icltable
				WHERE $wpdb->posts.post_type = 'authorship' AND $wpdb->posts.post_status != 'revision' AND $wpdb->posts.ID = {$db_icltable}.element_id AND {$db_icltable}.language_code = '$lang_code' AND ($search)
		...
		foreach ( $posts as $post ) {
			...
			if ( $wpml ) :
				...
				*/
				...
				$html .= '<td><label for="found-'.$post->ID.'">'.esc_html( $post->post_title ).'</label></td><td>'.esc_html( $lang_code ).'</td><td>'.esc_html( $time ).'</td><td>'.esc_html( $stat ).'</td></tr>'."\n\n";
			else :
				$html .= '<tr class="authors-found"><td class="found-radio"><input type="checkbox" id="found-author-'.$post->ID.'" name="found_author_id[]" value="' . esc_attr($post->ID) . '"></td>';
	...
	public function tasrpas_ajax_find_content()
	{
		...
		if ( class_exists('SitePress') && defined('ICL_LANGUAGE_CODE') ) :
			...
			$query_str = "
				SELECT $wpdb->posts.ID, $wpdb->posts.post_title, $wpdb->posts.post_status, $wpdb->posts.post_date, $wpdb->posts.post_type, {$db_icltable}.language_code
				FROM $wpdb->posts, $db_icltable
				WHERE $wpdb->posts.post_type IN ($what) AND $wpdb->posts.post_status != 'revision' AND $wpdb->posts.ID = {$db_icltable}.element_id AND {$db_icltable}.language_code = '$lang_code' AND ($search)
				ORDER BY $wpdb->posts.post_date_gmt DESC LIMIT 50
			";			
		else :
			$query_str = "
